...
const VerifyAccount = () => {
  ...
  useEffect(() => {
    const verifyUser = async () => {
      try {
        const response = await axios.get(`${BASE_URL + ENDPOINT.users}/verify/${token}`);
        setStatus("success");
        setMessage(response.data.message); 
      } catch (error) {
        setStatus("fail");
        setMessage(error?.response?.data?.message || "An error occurred while verifying your account");
      }
    };
